...
admin_user_id = 1  # Первый зарегистрированный пользователь станет админом
...
def pay_order(
    ...
    if not order or order.buyer_id != user.id:
        raise HTTPException(status_code=400, detail="Нельзя оплатить")
    order.status = "paid"
    db.commit()
    return RedirectResponse(f"/trade/{order.id}", status_code=302)

# -- Confirm (seller confirms and admin takes 0.5% commission) --
...
def confirm_order(
    ...
    if order.status != "paid":
        raise HTTPException(status_code=400, detail="Сначала отметьте оплату")

    commission = order.amount * 0.005
    seller_amount = order.amount - commission

    # Проводим списание/начисление
    if seller.balance < order.amount:
        raise HTTPException(status_code=400, detail="Недостаточно баланса у продавца")
