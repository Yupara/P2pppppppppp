...
class Ad(Base):
    ...
    payment_methods = Column(String, nullable=False)  # comma-separated
...
def market(
    ...
    q = db.query(Ad)
    if crypto:
        q = q.filter(Ad.crypto==crypto)
    if fiat:
        q = q.filter(Ad.fiat==fiat)
    if payment:
        q = q.filter(Ad.payment_methods.ilike(f"%{payment}%"))
    ads = q.all()
...
def my_orders(request: Request, db: Session = Depends(get_db)):
    ...
    orders = db.query(Trade).filter(Trade.buyer_uuid==uu).all()
