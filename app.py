...
# —— Константы ——
ADMIN_UUID             = "293dd908-eac9-4082-b5e6-112d649a60bc"  # ваш UUID админа
...
def mark_confirm(trade_id:int, request:Request, db:Session=Depends(get_db)):
    ...
    if not t or t.ad.uuid!=uu or t.status!="paid":
        raise HTTPException(403)
    # комиссия
    commission_buyer  = t.ad.amount * 0.005
    commission_seller = t.ad.amount * 0.005
    # участники
    buyer  = db.query(User).filter(User.uuid==t.buyer_uuid).first()
    seller = db.query(User).filter(User.uuid==t.ad.uuid).first()
    admin  = db.query(User).filter(User.uuid==ADMIN_UUID).first()
    # перевод средств
    total_amount = t.ad.amount
    buyer_net    = total_amount - commission_buyer
    seller_net   = total_amount - commission_seller
    seller.balance   += seller_net
    buyer.balance    -= total_amount  # списываем у покупателя всю сумму
    admin.balance    += (commission_buyer + commission_seller)
    t.status="confirmed"
    db.commit()
    return RedirectResponse(f"/trade/{trade_id}",302)
...
def deposit(request:Request, amount:float=Form(...), db:Session=Depends(get_db)):
    uu  = get_user_uuid(request); addr=str(uuid4())
...
def withdraw(request:Request, amount:float=Form(...),
             address:str=Form(...), db:Session=Depends(get_db)):
    uu   = get_user_uuid(request)
    user = db.query(User).filter(User.uuid==uu).first()
    if user.balance < amount: raise HTTPException(400,"Недостаточно средств")
    op   = Operation(user_uuid=uu,type="withdraw",amount=amount,address=address,status="pending")
    user.balance -= amount
    db.add(op); db.commit()
    return RedirectResponse("/profile",302)
...
def create_payment_session(request:Request,
    amount:float=Form(...), db:Session=Depends(get_db)
):
    uu = get_user_uuid(request)
    stripe_amt = int(amount*100)
    ...
    tx = PaymentTransaction(
        user_uuid=uu, stripe_session=session.id,
...
async def stripe_webhook(request:Request, db:Session=Depends(get_db)):
    ...
    if event["type"]=="checkout.session.completed":
        ...
        sid  = sess["id"]
        tx   = db.query(PaymentTransaction).filter_by(stripe_session=sid).first()
...
def admin_panel(request:Request, db:Session=Depends(get_db)):
    ...
    # список спорных сделок для ручного решения
    disputes = db.query(Trade).filter(Trade.status=="disputed").all()
