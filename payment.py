...
def market(request: Request,
    ...
    warning = "Ваш аккаунт заблокирован" if user.is_blocked else None
    ads = db.query(models.Ad).all()
    return templates.TemplateResponse("market.html", {"request": request, "ads": ads, "user": user, "warning": warning})

@router.get("/create_order/{ad_id}")
def create_order(ad_id: int,
                 request: Request,
                 db: Session = Depends(get_db),
                 user: models.User = Depends(get_current_user)):
    if user.is_blocked:
